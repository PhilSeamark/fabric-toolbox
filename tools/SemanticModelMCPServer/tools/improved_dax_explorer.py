...
def execute_dax_query(workspace_name: str, dataset_name: str, dax_query: str) -> dict:
    """
    Execute DAX query against Power BI Service model.
    This is an alias function for compatibility with chart tools.
    ...
    """
    try:
        # Use the core DAX execution logic directly
        import os
        import json
        ...
        # Build connection string with catalog parameter
        import urllib.parse
...
# Alias for local DAX execution (for enhanced_dax_explorer compatibility)
def execute_dax_query_local(connection_string: str, dax_query: str) -> dict:
    """
    Execute DAX query against local Power BI Desktop instance.
    This is an alias function for compatibility with enhanced_dax_explorer.
    ...
    try:
        ...
        return {
            'success': False,
            'error': f'Error executing local DAX query: {str(e)}',
            'connection_string': connection_string,
            'query': dax_query
        }